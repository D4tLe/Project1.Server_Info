...
public class SystemInfoTest { // NOSONAR squid:S5786
    ...
    private static void printProcesses(OperatingSystem os, GlobalMemory memory) {
        ...
        for (int i = 0; i < procs.size(); i++) {
            ...
            oshi.add(String.format(Locale.ROOT, " %5d %5.1f %4.1f %9d %9s %s", p.getProcessID(),
                    100d * (p.getKernelTime() + p.getUserTime()) / p.getUpTime(),
                    100d * p.getResidentSetSize() / memory.getTotal(), p.getVirtualSize(),
                    FormatUtil.formatBytes(p.getVirtualSize()), p.getName()));
        }
